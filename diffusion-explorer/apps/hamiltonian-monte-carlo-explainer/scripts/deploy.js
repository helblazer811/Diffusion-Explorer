...
const targetDir = path.join(tempDir, "blog/hamiltonian-monte-carlo");

try {
  console.log("Cloning main website repo...");
  execSync(`git clone ${mainRepoUrl} ${tempDir}`, { stdio: "inherit" });

  console.log("Cleaning old blog/hamiltonian-monte-carlo...");
  ...
  execSync(
    `cd ${tempDir} && git add blog/hamiltonian-monte-carlo && git commit -m "Update hamiltonian-monte-carlo blog" && git push`,
    { stdio: "inherit" }
  );
