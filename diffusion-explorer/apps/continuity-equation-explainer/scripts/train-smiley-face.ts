/**
 * Train Smiley Face Flow Model
 *
 * Trains a 2D flow matching model to transform Gaussian source → Smiley face target.
 * This model is shared by multiple figures (CrownJewel, FlowInvertibility, etc.)
 *
 * Usage: npx tsx scripts/train-smiley-face.ts
 *
 * Output:
 *   - static/smiley_face/models/flow_model.json
 *   - static/smiley_face/models/flow_model.weights.bin
 */
...
async function trainFlowModel(): Promise<string> {
  const config = CONFIG.model;

  console.log('\n========================================');
  console.log('  Smiley Face Flow Model Training');
  console.log('========================================\n');

  const startTime = Date.now();

  // Load dataset (use raw data, no normalization)
...
async function main() {
  ...
  console.log('\n****************************************');
  console.log('  Smiley Face - Model Training');
  console.log('****************************************\n');
