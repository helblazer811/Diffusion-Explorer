...
<script lang="ts">
  ...
  export let allTimeSamples: Writable<[number, number][][]>;
  ...
  async function initGPUContours() {
    ...
    try {
      ...
      console.log(`[ProbabilityPathIntro] ContourRenderer using ${contourRenderer.backend} backend`);
      ...
      console.log(`[ProbabilityPathIntro] Pre-computing ${allSamples.length} contour frames...`);
      const startTime = performance.now();
      await contourRenderer.computeAllFrames();
      const elapsed = performance.now() - startTime;
      console.log(`[ProbabilityPathIntro] Contour frames computed in ${elapsed.toFixed(1)}ms`);

      contoursReady = true;
    } catch (e) {
      console.warn('[ProbabilityPathIntro] GPU contour init failed, falling back to CPU:', e);
