...
const CONFIG = {
  ...
  modelPath: 'static/diffusion_horizontal/models/diffusion_model.json',
  modelConfig: {
    dim: 2,
    hidden: 128,
    T: 1000,
  ...
  ddpm: {
    outputPath: 'static/diffusion_horizontal/cached_samples/ddpm_trajectories.json',
    numSteps: 200,
...
async function generateTrajectories(
  ...
  console.log('\n----------------------------------------');
  console.log('  DDPM Trajectories');
  console.log('----------------------------------------');

  const startTime = Date.now();

  console.log(`  Grid: ${CONFIG.gridResolution}x${CONFIG.gridResolution}`);
  console.log(`  Steps: ${CONFIG.ddpm.numSteps}`);
  console.log(`  Scheduler: ${CONFIG.ddpm.scheduler}`);

  // Generate grid trajectories with DDPM scheduler
  const trajectories = await model.sample_grid(
    CONFIG.gridResolution,
    CONFIG.domainRange,
    CONFIG.ddpm.numSteps,
    { scheduler: CONFIG.ddpm.scheduler }
  );
  ...
  if (trajectories) {
    ...
    const outputFile = path.join(ROOT, CONFIG.ddpm.outputPath);
    ...
    console.log(`  Saved to: ${CONFIG.ddpm.outputPath}`);
...
async function main() {
  ...
  console.log('\n****************************************');
  console.log('  Diffusion Horizontal - Sample Caching');
  console.log('****************************************\n');
  ...
  if (!fs.existsSync(modelJsonPath)) {
    ...
    console.error('Please run: npm run train:diffusion-horizontal');
    process.exit(1);
  }
  ...
  // Generate DDPM trajectories
  await generateTrajectories(model);
