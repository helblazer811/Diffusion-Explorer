...
const CONFIG = {
  ...
  modelOutputDir: 'static/flow_model_dlic/models',
  dim: 2,
  hidden: 64,
  epochs: 2000,
  batchSize: 1024,
...
// Train Flow Model with Gaussian source distribution
async function trainFlowModel(targetData: tf.Tensor2D): Promise<string> {
  console.log('\n========================================');
  console.log('  Training Flow Model: GAUSSIAN source');
  console.log('========================================\n');

  const startTime = Date.now();
  const numSamples = targetData.shape[0];

  // Generate Gaussian source distribution (same size as target for coupling)
  console.log(`Generating Gaussian source distribution (${numSamples} samples)`);
  const sourceDistribution = tf.randomNormal([numSamples, 2]);
  ...
  // Train with Gaussian source distribution
...
async function main() {
  ...
  console.log('\n****************************************');
  console.log('  Flow Model DLIC - Model Training');
  console.log('****************************************\n');
  ...
  // Train model
  await trainFlowModel(targetData);
