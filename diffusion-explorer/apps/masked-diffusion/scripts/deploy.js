...
const targetDir = path.join(tempDir, "blog/masked-diffusion");

try {
  console.log("Cloning main website repo...");
  execSync(`git clone ${mainRepoUrl} ${tempDir}`, { stdio: "inherit" });

  console.log("Cleaning old blog/masked-diffusion...");
  ...
  execSync(
    `cd ${tempDir} && git add blog/masked-diffusion && git commit -m "Update masked-diffusion blog" && git push`,
    { stdio: "inherit" }
  );
