...
export async function trainFlowMatching(config = CONFIG): Promise<string> {
  // Initialize backend
  await initBackend();
  ...
  // Training callback for verbose output
  ...
  // Save model - access internal model via (model as any).model
